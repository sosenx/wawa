...
if ( ! function_exists( 'ot_type_et_padding' ) ) {

  function ot_type_et_padding( $args = array() ) {
    ...
    echo '<div class="format-setting type-border ' . ( $has_desc ? 'has-desc' : 'no-desc' ) . '">';
      ...
      echo '<div class="format-setting-inner">';
        ...
        /* allow fields to be filtered */
        $ot_recognized_border_fields = apply_filters( 'ot_recognized_et_padding_fields', array(
          'padding-top',
          'padding-right',
          'padding-bottom',
          'padding-left',
        ...
        $in = array( 'px', 'em', 'rem', 'vw', 'vh', 'vmin', 'vmax', '%' );
          ...
          foreach ( $ot_recognized_border_fields as $value ) {

            if ( in_array( $value, $ot_recognized_border_fields ) ) {

              if ( isset( $field_value[$value] ) && ! empty( $field_value[$value] ) ) {
                $padding = et_check_units( $in, $field_value[$value] ) ? $field_value[$value] : $field_value[$value] . 'px';
              } else {
                $padding = '';
              }

              printf( '<div class="ot-option-group ot-option-group--one-sixth"><input type="text" name="%1$s[%2$s]" id="%3$s %2$s" value="%4$s" class="widefat option-tree-ui-input %3$s" " placeholder="%2$s"/></div>',
                esc_attr( $field_name ),
                esc_attr( $value ),
                esc_attr( $field_id ),
                esc_attr( $padding )
