...
add_shortcode('et_top_cart','et_top_cart_shortcode');
...
function et_top_cart_shortcode($atts) {
	...
	$style = ( $position == 'center' ) ? '.' . $rand . '{width: 100%;text-align: center; display:inline-block;}.' . $rand . ' .widget_shopping_cart_content{display:inline-block;}.' . $rand . ' .cart-popup-container:before{left: 20px;}.cart-popup-container{right: auto;}' : '.' . $rand . '{float:' . esc_attr( $position ) . ';}';

	return '<div class="shopping-container ' . $rand . esc_html( $class ) . '" ' . $favicon_badge . '><div class="widget_shopping_cart_content">' . $output . '</div></div><style type="text/css">' . $style . '</style>';
...
add_shortcode('et_menu','et_menu_shortcode');
...
add_shortcode('et_links','et_links_shortcode');
...
add_shortcode('et_promo','et_promo_shortcode');
...
function et_promo_shortcode($atts) {
	...
	$output .= '<div class="popup_link ' . $rand . '' . esc_html( $class ) . '"><a class="etheme-popup open-click" href="#etheme-popup">' . __( 'Newsletter', ETHEME_DOMAIN ) . '</a></div>';
