...
class Et_Navigation extends Walker_Nav_Menu {
    ...
    function start_el ( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if( $depth == 0) {
            ...
            if ( $design != 'dropdown' && $columns != '') {
               $classes[] = 'menu-column'.$columns;
            }
        } else {
        ...
        if( $icon != '' && !has_post_thumbnail( $item_id ) ) $icon = '<i class="fa fa-'.$icon.'"></i>';
        if( $label != '' ) $classes[]= 'label-'.$label;
        ...
        $link_classes = 'item-link';
        $link_classes .= ( $design != '' && $design == 'full-width open-by-click' ) ? ' opener' : '';
