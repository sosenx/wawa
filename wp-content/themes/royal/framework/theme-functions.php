    ...
    function etheme_init() {
        ...
        ?>
            ...
            <style type="text/css">
                ...
                <?php et_load_bg_option( etheme_get_option('fixed_header_bg'), '.fixed-header' ); ?>
            ...
            <script type="text/javascript">
                ...
                var errorAdded = '<?php echo esc_js(__('sorry you can\'t add this product to your cart', ETHEME_DOMAIN)); ?>';
...
if(!function_exists('et_load_bg_option')) {
    function et_load_bg_option( $value, $selector, $external = '' ) {
          ...
          if ( ! empty( $value['background-size'] ) )
            $size = et_check_units( $in, $value['background-size'] ) ? 'background-size: ' . $value['background-size'] . ';' : 'background-size: ' . $value['background-size'] . 'px;';
...
function etheme_add_admin_script(){
    ...
    wp_enqueue_style("font-lato",etheme_http()."fonts.googleapis.com/css?family=Lato:300,400,700,300italic");
}
            ...
            <ul class="links<?php echo ( isset( $args['newsletter'] ) && $args['class'] ) ? ' ' . $args['class'] : '' ; ?>">
				...
				<?php if(etheme_get_option('top_links')): ?>
	                ...
	                <?php else : ?>
	                	...
	                	<?php endif; ?>
	                    <?php if( ! empty( $reg_id ) || ( isset( $args['register'] ) && $args['register'] == true ) ): ?>
...
if(!function_exists('etheme_search_form')) {
    function etheme_search_form( $search_view = '', $class = '') {
        ...
        ?>
            <div class="header-search <?php echo $class; ?>">
...
if(!function_exists('et_send_msg_action')) {
    function et_send_msg_action() {
        ...
        if(isset($_GET['contact-submit'])) {
            ...
            if( ! $captcha_instance->check( $_GET['captcha-prefix'], $_GET['captcha-word'] ) ) $return['msg'][] = __( 'The security code you entered did not match. Please try again.', ETHEME_DOMAIN );

            if( trim( $_GET['contact-name'] ) === '') $return['msg'][] = __( 'fill in the "Name and Surname" field.', ETHEME_DOMAIN );

            if( trim( $_GET['contact-email'] ) === '' || !isValidEmail( $_GET['contact-email'] ) ) $return['msg'][] = __( 'Please enter a valid email.', ETHEME_DOMAIN );

            if( trim( $_GET['contact-msg'] ) === '') $return['msg'][] = __( 'fill in the "Message" field.', ETHEME_DOMAIN );
...
if(!function_exists('et_register_action')) {
	function et_register_action() {
		...
		if(!empty($_GET['et_register'])){
			...
			if( ! $captcha_instance->check( $_REQUEST['captcha-prefix'], $_REQUEST['captcha-word'] ) ) $return['msg'][] = __('The security code you entered did not match. Please try again.', ETHEME_DOMAIN);

			if( empty( $username ) ) $return['msg'][] = __( "User name should not be empty.", ETHEME_DOMAIN );

			if( ! preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/", $email ) )  $return['msg'][] = __( "Please enter a valid email.", ETHEME_DOMAIN );

			if( empty( $pass ) || strlen( $pass ) < 5 ) $return['msg'][] = __( "Password should have more than 5 symbols", ETHEME_DOMAIN );

			if( empty( $pass2 ) || strlen( $pass2 ) < 5 ) $return['msg'][] = __( "Re-enter password should have more than 5 symbols", ETHEME_DOMAIN );

			if( $pass != $pass2 ) $return['msg'][] = __( "The passwords do not match", ETHEME_DOMAIN );

			if ( email_exists( $email ) ) $return['msg'][] = __( "That email already exists. Please try another one.", ETHEME_DOMAIN );

			if ( username_exists( $username ) ) $return['msg'][] = __( "Username already exists. Please try another one.", ETHEME_DOMAIN );
			...
			<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
				<div class="media">
					...
					<?php if ( $rating && get_option( 'woocommerce_enable_review_rating' ) == 'yes' ) : ?>
						...
						<div class="star-rating" title="<?php echo sprintf( __( 'Rated %d out of 5', ETHEME_DOMAIN ), $rating ) ?>">
							<span style="width:<?php echo ( $rating / 5 ) * 100; ?>%"><strong><?php echo $rating; ?></strong> <?php _e( 'out of 5', ETHEME_DOMAIN ); ?></span>
						</div>
