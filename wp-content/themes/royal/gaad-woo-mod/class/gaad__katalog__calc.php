<?php 
/*
* Oblicza cenę katalogu
*/
class gaad__katalog__calc extends gaad_calc {	
	...
	*/
	function variation_exists ( ) {
		$variation_attr_array = gaad__katalog__calc::get_variation_attr_array();
	...
	*/
	function save_variation(){
		...
		$variation_id = gaad__katalog__calc::variation_exists();
		if( !$variation_id ){
	...
	*/
	...
	$termin = $termin == 0 ? 1 : $termin;
		return array(
			...
			*/
			'attribute_pa_termin-wykonania' => $termin, 
			'attribute_pa_naklad' => (int)str_replace( '-szt', '', ($_POST["post_data"]['attribute_pa_ilosc-stron-czarno-bialych'] / 2 + 1 )) . '-szt',		
			'attribute_pa_format' => $_POST["post_data"]["attribute_pa_format"]
	...
	function calculate_variant_price() {
		...
		*/
			...
			* Lakier wybiórczy okładki
			...
			if( filter_var( $_POST["post_data"]["attribute_pa_lakier-wybiorczy-okladki"], FILTER_VALIDATE_BOOLEAN ) ){
				...
				$total_price['spot_wrap2'] = $this->sheet_B3_spot_uv_price() / $_markup;
		...
		*/
		...
		return $GLOBALS['post_calc_data_tmp'] = array_merge( $total_price, array( 
			...
			'variation_attr' => gaad__katalog__calc::get_variation_attr_array()
			) );
