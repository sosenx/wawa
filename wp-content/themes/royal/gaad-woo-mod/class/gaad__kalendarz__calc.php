...
*/
class gaad__kalendarz__calc extends gaad_calc {	
	...
	$termin = $termin == 0 ? 1 : $termin;
		...
		//'attribute_pa_podloze' => $_POST["post_data"]["attribute_pa_podloze"],
			...
			*/
			...
			'attribute_pa_lakier-wybiorczy' => $_POST[ 'post_data' ][ 'attribute_pa_lakier-wybiorczy' ], 
	...
	**/
	function calculate_variant_price() {
		...
		*/
		...
		//echo '<pre>'; echo var_dump( $color_calculation ); echo '</pre>';
		...
		*/
		$total_price = array(
			'color' => $variant_production['color']["_regular_price"] * 2			
		);
		...
		*/
		...
		foreach( $total_price as $k => $v ){
			...
			return $GLOBALS['post_calc_data_tmp'] = array_merge( $total_price, array( 
				...
				'variation_attr' => gaad__kalendarz__calc::get_variation_attr_array()
				) );
