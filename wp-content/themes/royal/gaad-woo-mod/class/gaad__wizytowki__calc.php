...
*/
class gaad__wizytowki__calc extends gaad_calc {	
	...
	*/
    ...
    $attribute_pa_lakier = isset( $GLOBALS['post_calc_data']["attribute_pa_lakier-wybiorczy"] ) ? $GLOBALS['post_calc_data']["attribute_pa_lakier-wybiorczy"] : 
   ...
   if( is_null($attribute_pa_lakier) ){
     $attribute_pa_lakier = false;
   }
...
//'attribute_pa_zadruk' => 'dwustronnie-kolorowe-4x4-cmyk',
			...
			*/
			...
			'attribute_pa_lakier-wybiorczy' => $attribute_pa_lakier, 
	...
	function calculate_variant_price() {
		...
		*/
		...
		foreach( $total_price as $k => $v ){
			...
			//$total_price['_regular_price'] = $this->gaad_round( $_price );
			...
			return $GLOBALS['post_calc_data_tmp'] = array_merge( $total_price, array( 
				...
				'variation_attr' => gaad__wizytowki__calc::get_variation_attr_array()
				) );
